...
def render_main_view():
    clear_view()
    Button(tk, text="List all tasks", bg="blue", fg="white", command=render_list_tasks_view)\
        .grid(column=0, row=0, pady=20, padx=20)
    Button(tk, text="Create task", bg="green", fg="white", command=render_create_view)\
        .grid(column=1, row=0, pady=20, padx=20)
...
def create_task(name, description, priority, is_completed):
    ...
    task = {"name": name, "description": description,
            "priority": priority_mapper[priority], "is_completed": is_completed}
    tasks.append(task)
...
def render_create_view():
    ...
    task_description.grid(column=1, row=1)
    ...
    check_box = Checkbutton(tk, text="Is completed", variable=task_completed)
